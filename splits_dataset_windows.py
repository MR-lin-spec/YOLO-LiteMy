...
def parse_args():
    parser = argparse.ArgumentParser(description="半监督数据集划分工具 (Windows 风格路径版)")
...
def normalize_path(p):
    """
    将路径转换为当前操作系统的标准风格字符串。
    在 Windows 上返回带反斜杠的路径 (如 C:\data)，在 Linux/Mac 上返回带正斜杠的路径。
    """
    if p is None:
        return ""
    # Path.__str__() 会根据操作系统自动使用正确的分隔符
    # os.path.normpath 可以进一步清理多余的分隔符并确定最终格式
    return os.path.normpath(str(p))

def load_config(config_path):
    """加载并解析原始 YAML 配置"""
    # 使用 resolve() 获取绝对路径，适配当前系统
    ...
    if not root_path.is_absolute():
        # 如果 YAML 中是相对路径，则相对于 YAML 文件所在目录解析
...
def safe_remove_dir(dir_path):
    """安全删除目录，如果存在的话"""
    if dir_path.exists():
        # 使用 normalize_path 以便在 Windows 控制台显示反斜杠路径
        print(f"🧹 检测到旧目录 {normalize_path(dir_path)}，正在清除以确保数据纯净...")
        shutil.rmtree(dir_path)
        print(f"   -> 已清除 {normalize_path(dir_path)}")
...
def copy_files_with_labels(source_images_dir, source_labels_dir, img_list, dest_root, subdir_name):
    ...
    for img_name in img_list:
        ...
        if src_img:
            # shutil.copy2 接受 Path 对象，无需转为字符串，但转为字符串也没问题
            shutil.copy2(src_img, dest_images / src_img.name)
            count_img += 1
            
            src_lbl = Path(source_labels_dir) / f"{img_name}.txt"
            if src_lbl.exists():
                shutil.copy2(src_lbl, dest_labels / f"{img_name}.txt")
                count_lbl += 1
    return count_img, count_lbl

def generate_yaml(output_dir, root_path, train_sub, val_sub, names, nc, yaml_name, is_unlabel=False):
    """生成 YAML 文件 (路径风格适配当前操作系统)"""
    # 使用 normalize_path 确保在 Windows 上生成反斜杠路径
...
def main():
    ...
    print(f"--- YOLO 半监督数据集划分工具 (Windows 风格路径版) ---")
    ...
    print(f"📂 数据集根目录：{normalize_path(root)}")
    ...
    if labeled_imgs:
        ...
        print(f"💾 有标签数据已存入：{normalize_path(labeled_dest_path)} (图:{count_img}, 标:{count_lbl})")
    ...
    if unlabel_imgs:
        ...
        for img_name in unlabel_imgs:
            ...
            if src_img:
                shutil.copy2(src_img, dest_images / src_img.name)
                count_u += 1
        print(f"💾 无标签数据已存入：{normalize_path(unlabel_dest_path)} (图:{count_u})")

    # 生成 YAML
    # 注意：这里传入的 train_sub 是相对路径字符串拼接，normalize_path 会处理分隔符
    yaml_labeled_path, _ = generate_yaml(
        output_dir=output_yaml_dir, root_path=root,
        train_sub=f"{args.labeled_dir_name}/images", # 这里内部会被 normalize_path 处理
        val_sub=val_sub,
        names=cfg['names'], nc=cfg['nc'],
        yaml_name=args.labeled_yaml_name, is_unlabel=False
    )
    print(f"📄 已生成：{normalize_path(yaml_labeled_path)}")
    ...
    print(f"📄 已生成：{normalize_path(yaml_unlabel_path)}")
