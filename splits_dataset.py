...
def move_files(source_images_dir, img_list, dest_dir):
    ...
    for img_name in img_list:
        ...
        if src_file:
            shutil.copy(str(src_file), str(dest_path / src_file.name))
            moved_count += 1
    return moved_count
...
def generate_yaml(script_dir, root_path, unlabel_subdir, yaml_name):
    ...
    """
    u_sub = unlabel_subdir.replace('/', '\\')
    r_path = root_path.replace('/', '\\')
...
"""
    ...
    # 【关键修改】yaml_path 基于 script_dir (脚本所在目录)
...
def main():
    ...
    if unlabel_imgs:
        ...
        # 生成 YAML
        # train 路径是相对于 root 的相对路径字符串
        train_rel = f"{args.output_dir}\\images"
        
        # 【关键调用】传入 script_dir，确保 yaml 生成在脚本旁边
        generate_yaml(
